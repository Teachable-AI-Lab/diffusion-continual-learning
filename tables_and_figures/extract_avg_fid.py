#!/usr/bin/env python3
"""
Extract avg_fid across W&B runs.

Rules:
- Run names follow: {dataset}-{method}-{seed}.
  - method may contain dashes.
...
- long: run_name, dataset, method, seed, _step, task_id, avg_fid
- wide (default): dataset, method, seed, avg_fid@task0, avg_fid@task1, ...
"""
...
def parse_experiment_name(name: str) -> ParsedName:
	"""Parse `{dataset}-{method}-{seed}` allowing dashes in method.

	If the last token is not an integer, seed defaults to 123 and the last token
...
def iter_task_rows(run) -> Iterable[Dict]:
	...
	# scan_history returns a generator; materialize only what we need to save memory.
	for item in run.scan_history():
		# item can be an object that behaves like dict
		try:
			row = dict(item)
		except Exception:
			# Fallback: some versions expose `.to_json()`
			try:
...
def extract_for_run(run, verbose: bool = False) -> List[Dict]:
	...
	for row in iter_task_rows(run):
		...
		try:
			...
			# skip rows where task_id isn't integer-like
			if verbose:
		...
		results.append(
			{
				...
				# wandb.Run.project is a string (project name)
...
def main(argv: Optional[List[str]] = None) -> int:
	parser = argparse.ArgumentParser(description="Extract avg_fid at each task_id from W&B runs.")
	parser.add_argument(
		"--project-path",
		default= "ndrsn0208-georgia-institute-of-technology/iclr-2026-exps",
		type=str,
		help="W&B path 'entity/project' (e.g., ndrsn0208-georgia-institute-of-technology/iclr-2026-exps)",
	)
	# parser.add_argument("--entity", type=str, default=None, help="W&B entity (if not using --project-path)")
	...
	parser.add_argument(
		...
		choices=["wide", "long"],
		default="wide",
		help="Output format: wide -> one row per (dataset,method,seed) with avg_fid@taskN columns; long -> one row per task_id.",
	)
	parser.add_argument(
		"--fillna",
		type=float,
	...
	if args.verbose:
		print(f"Listing runs for {project_path} with filters={filters or {}}...")

	runs = api.runs(project_path, filters=filters)  # type: ignore[arg-type]
	...
	if not all_rows:
		...
		# Empty headers depending on format
		if args.format == "wide":
			df_empty = pd.DataFrame(columns=["dataset", "method", "seed"])  # no task columns until data exists
		else:
			df_empty = pd.DataFrame(columns=["run_name", "dataset", "method", "seed", "_step", "task_id", "avg_fid"])
	...
	# Clean up: ensure numeric types where reasonable
	...
	if args.format == "long":
		# Sort for readability: dataset, method, seed, task_id
		sort_cols = [c for c in ["dataset", "method", "seed", "task_id"] if c in df_long.columns]
		if sort_cols:
	...
	# Wide format: pivot to one row per (dataset, method, seed) with columns avg_fid@taskN
	dfw = df_long[["dataset", "method", "seed", "task_id", "avg_fid"]].copy()
	dfw = dfw.dropna(subset=["task_id"])  # require task_id
	# Aggregate duplicates (if any) by mean
	dfw = (
		dfw.groupby(["dataset", "method", "seed", "task_id"], dropna=False, as_index=False)["avg_fid"].mean()
	)
	pivot = dfw.pivot_table(
		index=["dataset", "method", "seed"],
		columns="task_id",
		values="avg_fid",
		aggfunc="mean",
	)
	...
	# Rename columns to avg_fid@taskN
