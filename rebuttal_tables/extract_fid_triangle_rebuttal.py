#!/usr/bin/env python3
"""
Build lower-triangular FID tables per run from W&B histories for top-eig experiments.

For each run name formatted as {dataset}-top-eig{num}-{seed}, iterate
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Create per-run lower-triangular FID tables from W&B histories (top-eig).")
    parser.add_argument(
        "--project-path",
    ...
    parser.add_argument("--out-dir", type=str, default="fid_triangles_rebuttal", help="Directory to write per-run CSV tables")
    ...
    for run in runs:
        ...
        # Write per-run CSV
        out_name = f"{parsed.dataset}_eig{parsed.num_eigs}_{parsed.seed}.csv"
        ...
        if args.combined_long and not df_tri.empty:
            ...
            df_tri_with_meta.insert(1, "num_eigs", parsed.num_eigs)
            df_tri_with_meta.insert(2, "seed", parsed.seed)
    ...
    if args.combined_long:
        combined = pd.concat(combined_frames, ignore_index=True) if combined_frames else pd.DataFrame(
            columns=["dataset", "num_eigs", "seed", "task_id"]
        )
