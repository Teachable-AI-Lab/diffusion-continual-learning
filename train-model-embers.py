...
def make_exp_path(args) -> Path:
    ...
    name = f"{args.dataset}-{args.ewc_fisher_type}{'-gr' if args.use_generative_replay else ''}{'-distil' if args.use_distillation else ''}-{args.seed}"
    exp_path = root / name
    ...
    for epoch in tqdm(range(start_epoch, num_epochs), desc=f"Task {task_id} - Epoch"):
        ...
        if run_wandb is not None:
            wandb.log({
                ...
                'loss/ewc': avg_ewc,
                'loss/kl': avg_kl,
                'epoch': epoch + num_epochs * task_id,
                'task_id': task_id,
            })
...
def main():
    ...
    # WandB (resumable with run_id stored alongside experiment)
    run = None
    if getattr(args, 'use_wandb', False):
        ...
        if run_id_file.exists():
            ...
            # Generate a random, unique run id on first run and persist it for resume
